...
export default function AvaliationView({ project }: Props) {
	const finalAval = project.avaliacao.barema;
	const renderRow = (model?: string) => {
		if (model == 'final') {
			return finalAval.map((item: IEvaluation2) => (
		...
		if (data.length > 0 && model == 'pre') {
			...
			return (
				<tr>
					<td>Medias:</td>
					<td>
						{data.map((item: any) => item.nota1).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota2).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota3).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota4).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota5).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota6).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota7).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td>
						{data.map((item: any) => item.nota8).reduce((soma: number, i: number) => soma + i) /
							quant}
					</td>
					<td className="text-orange-100 underline">
						{data
