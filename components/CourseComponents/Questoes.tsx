...
export default function Questoes({ id, exerciceId, index, data }: any) {
  ...
  const submit = (data: any) => {
    ...
    api
      ...
      .then((res) => {
        alert('Salvo com sucesso, caso não esteja vendo sua edição atualize a página.')
  ...
  const renderQuests = (item: any, index: number) => {
    if (item.type == 'mult') {
...
const OpenQuest = ({ index = 0, control }: any) => {
...
const MultQuest = ({ index = 0, control }: any) => {
  const questaoPrefix = `quest[${index}]`;
