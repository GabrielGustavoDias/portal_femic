...
export default function Atividade({ test, id, moduleId, index }) {
  const [selectedExercise, setSelectedExercise] = useState(0);
  const [exerciseCount, setExerciseCount] = useState(1);
  const [activities, setActivities] = useState([{ id: '2e', data: [] }]);
...
export function Aula({ id, exerciceId, index, data, concluido = false }) {
  const submit = () => {
    api
      .patch(
...
export const Canva = ({ id, exerciceId, index, data, concluido = false }) => {
  const submit = () => {
    api
      .patch(
        `/user/course/update/${id}`,
...
export const Texto = ({ id, exerciceId, index, data, concluido = false }) => {
  const alert = useAlert();
  const submit = () => {
    api
      .patch(
