...
export default function Questoes({ data }: any) {
  const [questoes, setQuestoes] = useState([]);
  const [respostasUsuario, setRespostasUsuario] = useState<any[]>([]);
  const [respostasErradas, setRespostasErradas] = useState<any[]>([]);
  ...
  const handleSubmit = (e: any) => {
    ...
    const respostas = questoes.map((questao: any) => {
      const resposta =
        questao.type === 'mult'
          ...
          {questao.type === 'mult' && (
            <div>
              {questao.alternativas.map(
                (alternativa: any, altIndex: number) => (
