...
const ProjectConfirmacao: NextPage<IProjectConfirm> = ({ project }) => {
	...
	return (
		<div className="flex flex-col gap-2">
			...
			<textarea className="rounded p-2 border" defaultValue={project.resume} rows={5}></textarea>
			<label>Palavras-chaves atualizadas</label>
			<span className="sub-label">{project.keywords.map((key) => key).join(',')}</span>
			...
			<span className="sub-label flex gap-4">
				<input className="h-8 px-2" type="url" value={url} onChange={(e) => setUrl(e.target.value)}  />
				<button onClick={updateUrl} className="text-black">atualizar</button>
			</span>
