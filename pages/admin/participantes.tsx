...
import { FormQuery, UserContainer, WelcomeHeader } from '../../styles/admin/styles.module';
...
export default function Participantes() {
	...
	const queryUsers = (e: any) => {
		e.preventDefault();
		setLoading(true);
		setUsers([]);
		api
			.get(
				`/admin/user/search/query?year=${e.target[0].value}&modality=${modality}&sex=${
					e.target[1].value
	...
	const searchForm = (e: any) => {
		...
		setUsers([]);
		api
			.patch(`/admin/users/find`, { method: e.target[0].value, value: e.target[1].value })
			.then((res) => {
				setUsers(res.data);
				setTotalUsers(1);
				setLoading(false);
	...
	const showDocIcon = (user: any) => {
		if (user.adult) {
			return <span style={{ fontSize: 12, color: '#777', marginLeft: 4 }}>+18</span>;
	...
	return (
		<AdminLayout>
			...
			{users.length > 0 &&
				users.map((user: any) => (
					<UserContainer key={user._id}>
						...
						<div className="flex flex-col">
							<span className="title flex flex-row items-center">Perfil{showDocIcon(user)}</span>
			...
			{users.length == 0 && <h1>Sem perfis de {renderModality(modality)} para mostrar</h1>}
			<Pagination
