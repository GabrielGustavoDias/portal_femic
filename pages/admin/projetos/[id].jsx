...
const ProjetoAdminView = (prop) => {
  ...
  const searchAvaliadores = (e, model) => {
    e.preventDefault();
    api
  ...
  const searchAvaliadoresByName = (e) => {
    e.preventDefault();
    api
      .patch(`/admin/users/find`, {
  ...
  const addAvaliador = (user, model) => {
    ...
    if (data.map((pre) => pre.user_avatar).includes(user.avatar_url)) {
  ...
  const removeAvaliador = (user, model) => {
    const data = model == 'pre' ? [...commissionPre] : [...commissionFinal];
  ...
  const renderRow = (model) => {
    ...
    return project.pre_avaliacao.barema.map((item) => (
      <tr key={item.avaliador}>
        <td className="!text-left">{item.avaliador_nome}</td>
        <td className="text-orange-700">{item.nota1}</td>
        <td className="text-orange-700">{item.nota2}</td>
          ...
          <TabPanel>
            {'recomendado,concluido,finalista,confirmado'.includes(
              ...
              <>
                ...
                {project.pre_avaliacao.barema.map((aval) => (
