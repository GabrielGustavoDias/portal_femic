...
export default function Rank() {
	...
	const [projectWin, setProjectWin] = useState<any>();
	...
	const openModal = (proj: IProject) => {
		setProjectWin(proj);
		setVisible(true);
	};

	const sendWinner = (e: any) => {
		e.preventDefault();
		api
	...
	const removePremiacao = (id: string) => {
		api
	...
	const renderRank = (rank: number, index: number) => {
	...
	return (
		<AdminLayout>
			...
			{projects.length > 0 &&
				projects.map((proj: IProject, index: number) => (
					<ProjectContainer avaliacao key={proj._id}>
						...
						<div className="flex flex-col">
							...
							<select>
								{proj.avaliacao?.barema?.map((aval: any) => (
