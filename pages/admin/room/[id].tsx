...
export default function Room({ room }: IRoomProps) {
  ...
  const router = useRouter();
	...
	const removeProject = (proj: IProject) => {
  ...
  const deleteRoom = () => {
	...
	return (
		<AdminLayout>
			<form className="form-alt" onSubmit={handleSubmit(saveRoom)}>
				...
				<label>Link</label>
				<input type="url" {...register('link')} style={{ width: 600 }} required />
				<h2>Projetos</h2>
				{projectsList.map((proj) => (
					<ProjectContainer key={proj._id}>
						<img src="/default.png" width={50} height={50} alt="Imagem do projeto" />
						...
						<div className="flex flex-col">
							...
							<span>
								{(proj?.instituition?.city || '...') + '/' + (proj?.instituition?.state || '...')}
							</span>
						</div>
						<div className="flex flex-col">
							<label>
							...
							<label>
								Participantes:{' '}
								<span className="!text-orange-700">{proj.team.length + proj.advisors.length}</span>
							</label>
						</div>
    ...
    <button className='px-3 py-2 text-white mx-20 my-30' onClick={deleteRoom} style={{backgroundColor: 'tomato'}}>Deletar Sala</button>
		</AdminLayout>
	);
}
