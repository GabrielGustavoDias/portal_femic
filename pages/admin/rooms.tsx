...
export default function Rooms() {
  ...
  const deleteAll = () => {
    const confirmation = confirm("Tem certeza que deseja deletar todas as salas?");

    if (confirmation) {

    api.delete('/rooms/dell/all').then(res => {
      console.log(res.data);
    }).catch(err => {
      console.warn(err);
    })
    ...
    <button className="delete ml-1">Deletar</button>
			...
			{rooms.map((room, index) => (
				<ProjectContainer key={room._id}>
					...
					<div className="flex">{/*<button className='delete'>Deletar sala</button> */}</div>
