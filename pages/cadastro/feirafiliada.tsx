...
export default function FeiraAfiliada() {
	...
	const [cityes, setCityes] = useState<[{ nome: string; id: number }] | []>([]);
		...
		<LayoutForm color1="#1B7824" color2="#51E717">
			{steps[currentStep].id === 'sing' && (
				<form
					...
					<Select onChange={(e) => setAbrangencia(e.currentTarget.value)} required>
						<option value="" disabled selected>
							Selecione aqui
						</option>
					...
					<label>Canais de divulgação da feira. Deixe sem preencher caso não possua.</label>
					<Input
						type="text"
					...
					<Input type="file" accept="image/*" onChange={(e) => fileSelected(e)} required />
					...
					<Input type="password" onChange={(e) => setPassCheck(e.target.value)} required />
			...
			{steps[currentStep].id === 'data' && (
				<form className="flex flex-1 flex-col p-10 h-full" onSubmit={submitFunction}>
					<h1 style={{ fontWeight: 600, marginBottom: 16, fontSize: 24 }}>Sobre a afiliação</h1>
					...
					<div className="flex flex-row items-center mb-3">
						<input className="mr-2" type="checkbox" onChange={(e) => setModJov(e.target.value)} />
						<label>FEMIC Jovem</label>
					</div>
					<div className="flex flex-row items-center mb-3">
						<input className="mr-2" type="checkbox" onChange={(e) => setModMais(e.target.value)} />
					...
					<div className="flex flex-row items-center mb-3">
						<input type="checkbox" className="mr-2" onChange={(e) => setAreaCBio(e.target.value)} />
						<label> Ciências Biológicas</label>
					</div>
					<div className="flex flex-row items-center mb-3">
						<input type="checkbox" className="mr-2" onChange={(e) => setAreaEng(e.target.value)} />
					...
					<div className="flex flex-row items-center mb-3">
						<input type="checkbox" className="mr-2" onChange={(e) => setAreaCSoc(e.target.value)} />
						<label> Ciências Sociais e Aplicadas</label>
					</div>
					<div className="flex flex-row items-center mb-3">
						<input type="checkbox" className="mr-2" onChange={(e) => setAreaSHum(e.target.value)} />
						<label>Ciências Humanas</label>
					</div>
					<div className="flex flex-row items-center mb-3">
						<input type="checkbox" className="mr-2" onChange={(e) => setAreaLing(e.target.value)} />
						<label>Linguística, Letras e Artes</label>
					</div>
					<h3>Quantidade de projetos previstos para o ano vigente?</h3>
					<Input type="number" onChange={(e) => setQuantProj(e.target.value)} required />
					<h3>Qual a data prevista para o início da sua feira no ano vigente?</h3>
					<Input type="date" onChange={(e) => setRealization(e.target.value)} required />
					...
					<h2 style={{ fontWeight: 600, marginBottom: 16, fontSize: 22 }}>Coordenação do evento</h2>
					...
					<Input onChange={(e) => setCordPhone(phoneMask(e.target.value))} value={cordPhone} />
					<div className="flex flex-row items-center ">
