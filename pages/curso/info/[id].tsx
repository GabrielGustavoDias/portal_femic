...
export default function CursoInfo() {
  ...
  const continueCourse = () => {
    if (user.courses.find((item: any) => item.id == curso._id)) {
      sessionStorage.setItem(
        'user_course',
        JSON.stringify(user.courses.find((item: any) => item.id == curso._id))
      );
      sessionStorage.setItem('course', JSON.stringify(curso));
      router.push(`/curso/${curso._id}`);
    } else {
  ...
  const openPDFInBrowser = (pdfData: any) => {
          ...
          <div className="flex items-start justify-between bg-white m-3 p-4 shadow-md rounded gap-4 max-w-[100%]">
            ...
            <div className="flex flex-col gap-4">
              ...
              <button
                ...
                {user.courses.find((item: any) => item.id == curso._id)
                  ? 'Continuar'
                  : 'Inscrever-me'}
              </button>
              {user.courses.find((item: any) => item.id == curso._id)
