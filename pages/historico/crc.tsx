...
export default function CrcHistorico() {
  ...
  useEffect(() => {
    ...
    api.get(`project/profile/${sessionStorage.getItem('profile_id')}`)
      .then(response => {
  ...
  const submitQuery = (e: any) => {
    e.preventDefault();
    api.get(`/affiliate/projects/query?year=${year}&state=${state}&modality=${modality}`, { headers: { Authorization: `Bearer ${sessionStorage.getItem("token")}` } })
      .then(res => {
  ...
  return (
    <LayoutBase title="Histórico">
      <ProjectList>
        {projects.length > 0 && projects.map((pro: any) => (
          <Project key={pro._id}>
            <img src={`https://apiportal.femic.com.br/project/logo/${pro._id}`} />
            ...
            <div className="flex flex-col">
              <label>Status</label>
              <span className="sub-label">{pro.crc.recommended?"Recomendado":"Não recomendado"}</span>
            </div>
          </Project>
        ))}
      </ProjectList>
