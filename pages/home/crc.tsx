...
export default function Crc() {
	...
	const searchProjects = (e: any) => {
		e.preventDefault();
		api
			.get(`/profiles/crc/query?&category=${e.target[0].value}&area=${e.target[1].value}`, {
				headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` },
			})
			.then((res) => {
			...
			return (
				<div className="flex flex-col items-center justify-center">
					<h1 className="text-4xl">
						{message ? message : 'Selecione uma categoria e uma área científica'}
					</h1>
					{message && (
						<div className="flex flex-col my-4 items-center justify-center">
							<span>
								Caso queira ver todos os projetos de todas as categorias e áreas, clique aqui:{' '}
							</span>
							<button
								onClick={showAll}
			...
			return (
				...
				projects.map((proj: IProject) => (
					<ProjectInfo style={{ marginBottom: 20 }} key={proj._id}>
						<Project>
							<div className="head">
								<img src={`${baseUrl}/project/logo/${proj._id}`} alt="Logo do projeto" />
							</div>
