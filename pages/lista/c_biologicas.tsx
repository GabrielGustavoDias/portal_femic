...
export default function ListaJovem({ projects }: Iprojects) {
  ...
  const requestAgain = (e: any) => {
    ...
    api
      .get(`${baseUrl}/project/lista/c_biologicas/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
	...
	return (
		<>
			<Head>
				<title>Ciências Biológicas {new Date().getFullYear()}</title>
			</Head>
			<NavBar />
			<ProjectContainerList style={{ marginTop: 75 }}>
				<h1>
					Projetos Ciências <b>biológicas</b>.
				</h1>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const projectData = await fetch(`${baseUrl}/project/lista/c_biologicas/${new Date().getFullYear() - 1}`);
