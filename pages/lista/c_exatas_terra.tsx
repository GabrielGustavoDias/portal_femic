...
export default function ListaJovem({ projects }: Iprojects) {
  ...
  const requestAgain = (e: any) => {
    ...
    api
      .get(`${baseUrl}/project/lista/c_exatas_terra/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
	...
	return (
		<>
			<Head>
				<title>Ciências Exatas e da terra {new Date().getFullYear()}</title>
			</Head>
			<NavBar />
			<ProjectContainerList style={{ marginTop: 75 }}>
				<h1>
					Projetos Ciências <b>exatas</b> e da <b>terra</b>.
				</h1>
        <FormQuery className="bg-slate-100 px-4 rounded py-2 gap-3" onSubmit={requestAgain}>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	const projectData = await fetch(`${baseUrl}/project/lista/c_exatas_terra/${new Date().getFullYear()- 1}`);
