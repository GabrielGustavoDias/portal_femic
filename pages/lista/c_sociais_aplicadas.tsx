...
export default function ListaJovem({ projects }: Iprojects) {
  ...
  const requestAgain = (e: any) => {
    e.preventDefault();
    api
      .get(`${baseUrl}/project/lista/c_sociais_aplicadas/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
	...
	return (
		<>
			<Head>
				<title>Ciências sociais aplicadas FEMIC {new Date().getFullYear()}</title>
			</Head>
			<NavBar />
			<ProjectContainerList style={{ marginTop: 75 }}>
				<h1>
					Projetos <b>Ciências</b> sociais aplicadas
				</h1>
				<FormQuery
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	const projectData = await fetch(`${baseUrl}/project/lista/c_sociais_aplicadas/${new Date().getFullYear()- 1}`);
