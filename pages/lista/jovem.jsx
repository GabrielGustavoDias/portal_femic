...
export default function ListaJovem({ projects }) {
  const [projs, setProjs] = useStateany(projects);

  const requestAgain = (e) => {
    e.preventDefault();
    api
      .get(`${baseUrl}/project/lista/jovem/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
  ...
  return (
    <>
      <Head>
        <title>Projetos FEMIC Jovem</title>
      </Head>
      <NavBar />
      <ProjectContainerList style={{ marginTop: 75 }}>
        <h1>
          Projetos <b>FEMIC</b> Jovem
        </h1>
        <FormQuery
...
export const getServerSideProps = async (context) => {
  const projectData = await fetch(
    `${baseUrl}/project/lista/jovem/${new Date().getFullYear() - 1}`
  );
