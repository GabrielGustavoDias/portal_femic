...
export default function ListaJovem({ projects }: Iprojects) {
  ...
  const requestAgain = (e: any) => {
    e.preventDefault();
    api
      .get(`${baseUrl}/project/lista/junior/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
	...
	return (
		<>
			<Head>
				<title>Projetos FEMIC Júnior</title>
			</Head>
			<NavBar />
			<ProjectContainerList style={{ marginTop: 75 }}>
				<h1>
					Projetos <b>FEMIC</b> júnior
				</h1>
				<FormQuery
...
export const getServerSideProps: GetServerSideProps = async (context) => {
	const projectData = await fetch(`${baseUrl}/project/lista/junior/${new Date().getFullYear()- 1}`);
