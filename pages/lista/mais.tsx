...
export default function ListaJovem({ projects }: Iprojects) {
  ...
  const requestAgain = (e: any) => {
    ...
    api
      .get(`${baseUrl}/project/lista/mais/${e.target[0].value}`)
      .then((res) => {
        setProjs(res.data || []);
      })
      .catch((err) => {
  ...
  return (
    <>
      <Head>
        <title>Projetos FEMIC mais</title>
      </Head>
      <NavBar />
      <ProjectContainerList style={{ marginTop: 75 }}>
        <h1>
          Projetos <b>FEMIC</b> Mais
        </h1>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  const projectData = await fetch(`${baseUrl}/project/lista/mais/${new Date().getFullYear()- 1}`);
