    ...
    api
      ...
      .then((res) => {
        ...
        sessionStorage.setItem(
          'avatar',
          `${baseUrl}/user/profile/image/${res.data.image}`
        );
        setAvatar(sessionStorage.getItem('avatar') || '');
        setVisible(false);
      })
  ...
  return (
    <LayoutBase title="Perfil">
        ...
        <div className="infos md:flex-row flex-col">
          <div className="flex flex-col name">
            ...
            <h3 className="text-base mb-1 text-slate-500">
              Participante desde{' '}
              {new Date(watch('singUp_date')).toLocaleDateString()}
            </h3>
          </div>
      ...
      <UpdateProfileForm onSubmit={handleSubmit(updateProfile)}>
        ...
        <div className="flex items-center">
          <Input
            placeholder={`${watch(
              'school'
            )} (Este item não pode ser modificado)`}
            disabled
          />
        ...
        <SelectMult
          ...
          defaultValue={formationTypes.filter((e: any) =>
            watch('formation')?.includes(e.label)
          )}
          ...
          onChange={(e: any) =>
            setValue('formation', e.map((it: any) => it.label).join())
          }
        />
        ...
        {watch('especial') == 's' && (
          <>
            ...
            <SelectMult
              placeholder="Selecione"
              defaultValue={defsTypes.filter((e: any) =>
                watch('especial_type')?.includes(e.label)
              )}
              closeMenuOnSelect={false}
              components={animatedComponents}
              isMulti
              ...
              onChange={(e: any) =>
                setValue('especial_type', e.map((it: any) => it.label).join())
              }
            />
          </>
        )}
        ...
        {watch('fluent') && (
          <SelectMult
            placeholder="Selecione"
            defaultValue={idiomasTypes.filter((e: any) =>
              watch('fluent')?.includes(e.label)
            )}
            closeMenuOnSelect={false}
            components={animatedComponents}
            isMulti
            ...
            onChange={(e: any) =>
              setValue('fluent', e.map((it: any) => it.label).join())
            }
          />
        )}
        <button
      ...
      <Link href="/desativar" passHref>
        <a className="text-red-500">Desativar minha conta</a>
      </Link>
    </LayoutBase>
  );
}
