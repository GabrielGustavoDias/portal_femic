...
export default function ProjetoInstituicao() {
	...
	useEffect(() => {
		...
		stateChanged('MG');
		api.get(`/project/instituition/${sessionStorage.getItem('project_id')}`).then((res) => {
			reset(res.data);
	...
	const updateInstituicao = (data: any) => {
			...
			<Form onSubmit={handleSubmit(updateInstituicao)}>
				...
				<select {...register('country')} required>
					{countryes &&
						countryes.map((paisItem) => (
							<option key={paisItem['pais-ISO-ALPHA-3']} value={paisItem['pais-nome']}>
								{paisItem['pais-nome']}
							</option>
						))}
				</select>
				...
				<Link href="/projetos/instituicaoparceira" passHref>
					<button className="add-insti">
						<FiPlusSquare color="#333" size={24} /> Adicionar instituição parceira
					</button>
				</Link>
