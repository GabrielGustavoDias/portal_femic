...
export default function ProjetoInstituicaoParceira() {
  ...
  useEffect(() => {
    ...
    api
      .get(
        `/project/instituition/parceira/${sessionStorage.getItem('project_id')}`
      )
      .then((res) => {
        reset(res.data);
  ...
  const updateInstituicaoPar = (data) => {
    data['id'] = sessionStorage.getItem('project_id');
    api.post('/project/instituition/parceira', data).then((res) => {
