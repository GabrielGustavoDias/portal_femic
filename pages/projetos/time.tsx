...
import { ListUsers, Seach, UserCard } from '../../styles/projetos/styles.module';
import CardPeople from '../../components/CardPeople';

export default function ProjetoTime() {
	const [projId, setProjId] = useState('');
	const [modality, setModality] = useState('');
	const [teamList, setTeamList] = useState<any[] | string[]>([]);
	const [orientadoresList, setOrientadoresList] = useState<any[] | []>([]);
	...
	const addTeamPart = () => {
		...
		if (teamList.length == 6 && (modality == 'orientador' || modality == 'mais')) {
			alert.show('Limite máximo de participantes.');
			setInputTeam('');
			return;
		}
		...
		api
			.get(`/profiles/user/${sessionStorage.getItem('modality_project')}?identifier=${inputTeam}`)
			.then((response) => {
				setInputTeam('');
				if (!response.data) {
					alert.show(
	...
	const removeTeamOri = (ori: any) => {
		const listAux = orientadoresList.slice();
		listAux.splice(listAux.indexOf(ori), 1);
		api.patch('/project/remove/team', { project: projId, user: ori._id }).then((res) => {
	...
	const submitUpdateTeam = () => {
		...
		if ((modality == 'jovem' || modality == 'junior') && orientadoresList.length < 1) {
	...
	return (
		<LayoutBase title="Forme sua Equipe">
			<span className="alert-text ml-7 mb-7">
				Para formar sua equipe, digite o CPF (brasileiros) ou o passaporte (estrangeiros) dos
				autores e/ou autoras do projeto. Use somente letras e números.
			</span>
			<ListUsers>
				{teamList &&
					teamList.map((integrante, index) => (
						<CardPeople key={integrante._id} integrante={integrante}>
							{sessionStorage.getItem('profile_id') !== integrante._id ? (
								<button className="remove" onClick={() => removeTeamUser(integrante)}>
									Remover
								</button>
							) : (
								<button className="remove" onClick={() => removeTeamUser(integrante)}>
			...
			{(modality == 'jovem' || modality == 'junior' || modality == 'orientador') && (
				<>
					<ListUsers>
						{orientadoresList &&
							orientadoresList.map((orientador) => (
								<CardPeople key={orientador._id} integrante={orientador}>
									<button className="remove" onClick={() => removeTeamOri(orientador)}>
										{sessionStorage.getItem('profile_id') == orientador._id ? 'Recusar' : 'Remover'}
									</button>
								</CardPeople>
							))}
					</ListUsers>
