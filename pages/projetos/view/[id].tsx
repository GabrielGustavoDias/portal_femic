...
import { Project, ProjectDetails, ProjectInfo } from '../../../styles/home/style.module';

import LayoutBase from '../../../styles/layout/base';
import { ListUsers, UserCard } from '../../../styles/projetos/styles.module';

import { renderModalityInstituition, renderAreaProject } from '../../../utils/projects';

import AvaliationView from '../../../components/AvaliationView';

const ProjetoView: NextPage = (prop: any) => {
				...
				<ListUsers>
					{team.team.map((part: any) => (
						<UserCard key={part._id}>
							<img src={`${baseUrl}/user/profile/image/${part.user_avatar}`} alt={part.user_name} />
					...
					{team.advisors?.map((adv: any) => (
						<UserCard key={adv._id}>
							<img src={`${baseUrl}/user/profile/image/${adv.user_avatar}`} alt={adv.user_name} />
							<h1>{adv.user_name}</h1>
					...
					<div className="details">
						...
						{showInst && project.instituition && (
							<div className="details-it">
								<label>Pais</label>
								<span className="sub-label">{project.instituition.country}</span>
								<label>Estado</label>
								...
								<span className="sub-label">{project.instituition.adm_category}</span>
					...
					{project.instituition_par && (
						<div className="details">
							...
							{showInst && (
								<div className="details-it">
									<label>Pais</label>
									<span className="sub-label">{project.instituition_par.country}</span>
									<label>Estado</label>
									<span className="sub-label">{project.instituition_par.state}</span>
									<label>Cidade</label>
									<span className="sub-label">{project.instituition_par.city}</span>
									<label>Nome</label>
									<span className="sub-label">{project.instituition_par.name}</span>
									<label>Categoria Administrativa</label>
									<span className="sub-label">
										{renderModalityInstituition(project.instituition_par.adm_category)}
									</span>
									<label>Telefone</label>
									<span className="sub-label">{project.instituition_par.number}</span>
									<label>E-mail</label>
									<span className="sub-label">{project.instituition_par.email}</span>
								</div>
							)}
						</div>
					)}
							...
							<div className="details-it">
								...
								{project.etica.involve_human.check == 's' && (
									<div className="ml-4 flex flex-col">
										<span className="sub-label">
											Seu projeto pode ser considerado uma pesquisa de opinião, na qual dados
											pessoais dos participantes (Ex: nome, idade, endereço, posses ou qualquer dado
											que identifique a pessoa) não são coletados e/ou divulgados:{' '}
											{project.etica.involve_human['01'] == 's' ? 'Sim' : 'Não'}
										</span>
										<span className="sub-label">
											2. Os participantes foram esclarecidos sobre a pesquisa e registraram sua
											concordância de participação através de um documento impresso ou virtual (Ex:
											Termo de Consentimento Livre e Esclarecido - TCLE; Termo de Assentimento -
											TA): {project.etica.involve_human['02'] == 's' ? 'Sim' : 'Não'}
										</span>
										<span className="sub-label">
										...
										<span className="sub-label">
											A pesquisa envolveu entrevistas gravadas seja apenas áudio (podcast) ou áudio
											e vídeo (vodcast): {project.etica.involve_human['04'] == 's' ? 'Sim' : 'Não'}
										</span>
										<div className="ml-4 flex flex-col">
											<span className="sub-label">
												Teste de produtos ou conceitos desenvolvidos pelos pesquisadores que não
												constituam riscos aos participantes por não haver a experimentação por
											...
											<span className="sub-label">
												...
												estatísticas de jogos, índice de crimes, etc.) desde que não requeira
												interação com as pessoas das quais os dados foram coletados:{' '}
												{project.etica.involve_human['05'].box3 ? 'Sim' : 'Não'}
											</span>

											<span className="sub-label">
												Estudos de análise de comportamento em espaços públicos, desde que os
												pesquisadores não interajam com os indivíduos estudados, não manipulem o
												ambiente e não registrem informações que os identifiquem:{' '}
												{project.etica.involve_human['05'].box4 ? 'Sim' : 'Não'}
											</span>

											<span className="sub-label">
												Coleta de material biológico (Ex: sangue, urina, fezes, raspado jugal,
												cabelo, unha, outros):{' '}
												{project.etica.involve_human['05'].box5 ? 'Sim' : 'Não'}
											</span>

											<span className="sub-label">
												Experimentação de produtos do projeto, por ingestão ou uso tópico (Ex:
												produtos alimentícios de origem animal ou vegetal, creme, shampoo, outros):{' '}
												{project.etica.involve_human['05'].box6 ? 'Sim' : 'Não'}
											</span>

											<span className="sub-label">
												{' '}
												Análise de comportamento humano em grupos considerados vulneráveis,
												independente de ser uma condição física, psicológica ou social, utilizando
												metodologia presencial ou virtual:{' '}
								...
								{project.etica.involve_agbio.check == 's' && (
									<div className="ml-4 flex flex-col">
										...
										<span className="sub-label">
											Partes de órgãos ou fluidos frescos cuja necropsia seja realizada para a
											pesquisa: {project.etica.involve_agbio.box9 ? 'Sim' : 'Não'}
										</span>
										<span className="sub-label">
											Tecnologias de recombinação gênica em que os microrganismos sofrem
											modificações genéticas: {project.etica.involve_agbio.box10 ? 'Sim' : 'Não'}
										</span>
								...
								{project.etica.involve_quimic.check == 's' && (
									<div className="flex flex-col ml-4">
										...
										<span className="sub-label">
											Como foi descartada a substância química ou seus rejeitos: <br />{' '}
						...
						{showFinal && project && (
							<div className="details-it">
								...
								<label className="mb-2">
									<input type="checkbox" disabled checked={project.status !== 'desistente'} />
									<span className="sub-label ml-2">
										Declaramos que o trabalho submetido para participação na FEMIC (Feira Mineira de
										Iniciação Científica) é original, de única e exclusiva autoria das pessoas
										informadas como membros(as) da equipe e não se trata de cópia integral ou
								...
								<label className="mb-2">
									<input type="checkbox" disabled checked={project.status !== 'desistente'} />
									<span className="sub-label ml-2">
										Declaramos ter total conhecimento e compreensão do que é considerado plágio, não
										apenas a cópia integral do trabalho, mas também de parte dele, inclusive de
										artigos e/ou parágrafos, sem citação do autor ou de sua fonte.{' '}
									</span>
								</label>
								<label className="mb-2">
									<input type="checkbox" disabled checked={project.status !== 'desistente'} />
				...
				{project?.avaliacao.total > 0 && project?.status == 'concluido' && (
					<div className="flex flex-1 w-full">
						<div className="details">
							...
							{showAvaliation && project && <AvaliationView project={project} />}
						</div>
					</div>
				)}
			</ProjectDetails>
