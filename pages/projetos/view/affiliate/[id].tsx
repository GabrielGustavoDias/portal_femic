...
import { ProjectInfo, Project, ProjectDetails } from "../../../../styles/home/style.module";
...
const ProjetoViewAffiliate: NextPage = (prop: any) => {
  ...
  useEffect(() => {
    api.get(`/project/team/${project._id}`)
      .then(res => {
        setTeam(res.data);
      })
  }, []);

  const confirmCredential = (e: any) => {
    e.preventDefault();
    const confirm = cred == "s" ? true : false;
    api.patch("/affiliate/credential/confirm", { project: project._id, confirm }, 
  ...
  return (
    <LayoutBase title="Projeto">
      <ProjectInfo>
        <Project>
          <div className="head">
            <img src={`https://apiportal.femic.com.br/project/logo/${project._id}`} />
          </div>
        </Project>
        ...
        <ListUsers>
          {team.team.map((part: any) => (
            <UserCard key={part._id}>
              <img src={`https://apiportal.femic.com.br/user/profile/image/${part.user_avatar}`} alt={part.user_name} />
          ...
          {team.advisors?.map((adv: any) => (
            <UserCard key={adv._id}>
              <img src={`https://apiportal.femic.com.br/user/profile/image/${adv.user_avatar}`} alt={adv.user_name} />
              <h1>{adv.user_name}</h1>
            ...
            <div className="details">
              ...
              {showInst && (
                <div className="details-it">
                  ...
                  <label>Categoria Administrativa</label>
                  <span className="sub-label">{project.instituition_par.adm_category}</span>
                  <label>Telefone</label>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const projectData = await fetch(`https://apiportal.femic.com.br/project/getserverside/${id}`);
