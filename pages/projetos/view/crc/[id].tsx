...
import { Project, ProjectDetails, ProjectInfoAdmin } from '../../../../styles/home/style.module';
...
const ProjetoView: NextPage = (prop: any) => {
	...
	useEffect(() => {
		if (project.status !== 'em-avaliacao') {
			alert.error('Este projeto já foi avaliado ou não está apto para entrar em avaliação CRC');
			router.back();
		}
	...
	const authorize = () => {
		api
			...
			.then((res) => {
				alert.show(
					`Projeto ${recommendation == 's' ? 'recomendado' : 'não recomendado'} com sucesso`
				);
				router.back();
			})
			.catch((err) => {
				alert.error('Ocorreu um erro ao salvar o projeto, faça login novamente');
				router.push('/login');
			});
	};
