  ...
  return (
    ...
    <div className="flex flex-col md:px-20 px-5 py-10 bg-slate-100">
      ...
      <form className="flex flex-col gap-2 items-start" onSubmit={reativacao}>
        ...
        <input type="password" className="w-64 px-2 rounded h-8 mb-4" />
        <span>Quando você pressionar o botão abaixo, seus projetos, histórico de participação, certificados e autorizações ficarão ativos novamente.</span>
    <label htmlFor="check" className="">
    <input type="checkbox" id="check" className="mx-2"/>
  Concordo com as <Link href="https://femic.com.br/politica-de-privacidade/">regras e políticas</Link> de privacidades da FEMIC
    </label>  
        <button className="text-white py-4 px-6 rounded bg-emerald-400 font-bold">Reativar conta</button>
      </form>
    </div>
    </div>
  );
