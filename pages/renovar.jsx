...
export default function Renovar() {
  ...
  const checkProfile = (value) => {
    setProf(value.split(';')[1]);
    setModality(value.split(';')[1]);
    setSchool(value.split(';')[0]);
  };
  const especializationSelected = (e) => {
    const test = e.map((item) => item.label);
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-col md:px-20 px-5 py-10 bg-slate-100">
        ...
        <form
          className="flex my-5 flex-col gap-2 items-start"
          onSubmit={reativacao}>
          ...
          <SelectMult
            ...
            onChange={(e) => setFluent(e.map((it) => it.label).join())}
          />
